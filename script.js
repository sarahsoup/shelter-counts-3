...
d3.csv(link, parse, function(err,counts){
  ...
  const countsIntake = counts.filter(function(d){ return d.cat == 'intake'});
  const countsOutcome = counts.filter(function(d){ return d.cat == 'outcome'});

  //determine temporal counts
  countsIntake.forEach(function(d){
    d.count = Math.round(d.count/366);
    // d.count = Math.round(d.count/(366*24));
  });
  countsOutcome.forEach(function(d){
    d.count = Math.round(d.count/366);
    // d.count =Math.round(d.count/(366*24));
  });
